...
REGISTRY ?= docker.io
REPO ?= $(REGISTRY)/wodby/laravel-php
NAME = laravel-php-$(PHP_VER)
...
test:
	...
	docker run --rm --network none --entrypoint /bin/bash -v "$(CURDIR)/tests/workspace-image-runtime.sh:/tmp/workspace-image-runtime.sh:ro" $(REPO):$(TAG) /tmp/workspace-image-runtime.sh
